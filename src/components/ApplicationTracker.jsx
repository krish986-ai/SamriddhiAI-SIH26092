        ...
        {selectedApp && (
          <div className="gov-card" style={{ padding: '1.8rem', background: '#ffffff', border: '1px solid #fed7aa' }}>
            ...
            <div style={{ background: '#fff7ed', padding: '1.2rem', borderRadius: '10px', border: '1px solid #fed7aa' }}>
              ...
              {selectedApp?.uploadedDocuments && (
                <div style={{ marginTop: '0.8rem', paddingTop: '0.8rem', borderTop: '1px solid #fed7aa' }}>
                  <div style={{ fontSize: '0.78rem', fontWeight: 700, color: '#9a3412', marginBottom: '0.4rem' }}>
                    Uploaded Applicant Dossier Files:
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', fontSize: '0.75rem' }}>
                    <span className="badge badge-orange">📄 Income: {selectedApp.uploadedDocuments.incomeDoc}</span>
                    <span className="badge badge-orange">📜 Caste: {selectedApp.uploadedDocuments.casteDoc}</span>
                    <span className="badge badge-orange">👤 Photo: {selectedApp.uploadedDocuments.photoDoc}</span>
                    <span className="badge badge-orange">🪪 ID: {selectedApp.uploadedDocuments.identityDoc}</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div style={{
          ...
          <div className="gov-card" style={{ maxWidth: '620px', width: '100%', padding: '2rem', background: '#ffffff', border: '2px solid #fed7aa' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
              ...
              <button 
                onClick={() => { 
                  const content = `GOVERNMENT OF INDIA - MINISTRY OF SOCIAL JUSTICE & EMPOWERMENT\nNATIONAL SCHEDULED CASTES FINANCE AND DEVELOPMENT CORPORATION (NSFDC)\nOFFICIAL SANCTION ORDER\nOrder Ref: #NSFDC-SO-2026-9921\nApplication ID: ${selectedApp?.appId || 'APP-SIH-2026'}\nBeneficiary: ${selectedApp?.applicantName || 'Sunita Devi'}\nScheme: ${selectedApp?.schemeTitle || 'Mahila Samriddhi Yojana'}\nSanctioned Amount: Rs. ${Number(selectedApp?.requestedAmount || 120000).toLocaleString('en-IN')}\nConcessional Interest Rate: 4.0% p.a.\nNodal Partner: ${selectedApp?.channelPartner || 'State Channelizing Agency'}\nStatus: SANCTIONED & READY FOR DBT DISBURSEMENT\n`;
                  const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
                  const url = URL.createObjectURL(blob);
                  const link = document.createElement('a');
                  link.href = url;
                  link.download = `Sanction_Order_${selectedApp?.appId || 'SIH2026'}.txt`;
